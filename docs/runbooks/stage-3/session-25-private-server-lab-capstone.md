...
By end of session, learner can run a controlled local/private server session from setup to shutdown.
...
- Core Quest (18 min): Host a tiny private build challenge for mentor or peer testers inside the prepared lab.
...
Start server, connect, and verify the right world loaded. This must land within the first 7 minutes so the learner feels oriented before touching harder server concepts.
...
- Debug: learner resolves or correctly diagnoses the prepared capstone over-scope or unsafe access.
...
“My server worked because ___. One issue I checked was ___.”
