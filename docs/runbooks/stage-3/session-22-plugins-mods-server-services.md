...
By end of session, learner can distinguish mods, plugins, and supporting services in a controlled server folder.
...
- Core Quest (18 min): Classify prepared features as mod, plugin, service, or config.
...
Enable one approved server-side feature and observe a visible change. This must land within the first 7 minutes so the learner feels oriented before touching harder server concepts.
...
- Debug: learner resolves or correctly diagnoses the prepared feature installed in wrong layer.
