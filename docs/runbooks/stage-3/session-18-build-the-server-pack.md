...
By end of session, learner can understand that a server pack is a controlled compatible file set, not a random folder dump.
...
- Core Quest (18 min): Inspect a small approved server pack matching the Stage 2 modpack.
...
Open a prepared server pack folder and identify mods, config, world, and logs. This must land within the first 7 minutes so the learner feels oriented before touching harder server concepts.
...
- Debug: learner resolves or correctly diagnoses the prepared missing or extra server file.
...
“A server pack needs ___ because ___.”
