...
By end of session, learner can understand that server behavior and world state live in separate files.
...
- Core Quest (18 min): Map the server folder: world, configs, logs, properties, and backups.
...
Change a harmless prepared setting, restart, and observe the result. This must land within the first 7 minutes so the learner feels oriented before touching harder server concepts.
...
- Debug: learner resolves or correctly diagnoses the prepared config versus world-state confusion.
...
“The world data lives in ___. The server settings live in ___.”
