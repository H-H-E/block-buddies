...
By end of session, learner can understand a server as a process that starts, runs, logs, accepts connections, and stops.
...
- Core Quest (18 min): Connect from a local client using a prepared local address.
...
Start a prepared local server and find the ready message. This must land within the first 7 minutes so the learner feels oriented before touching harder server concepts.
...
- Debug: learner resolves or correctly diagnoses the prepared server-not-running false network blame.
...
“Before anyone can connect, the server must be ___.”
