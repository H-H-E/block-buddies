...
By end of session, learner can understand that some mods run on the client, some on the server, and some must match on both sides.
...
- Core Quest (18 min): Compare a prepared client pack and server pack; identify which files belong where.
...
Sort approved mod cards into client, server, and both. This must land within the first 7 minutes so the learner feels oriented before touching harder server concepts.
...
- Debug: learner resolves or correctly diagnoses the prepared mod placement mismatch.
...
“One mod that belongs on the client is ___. One mod that belongs on the server is ___.”
