...
By end of session, learner can learn safe access control: who can join, who can use admin powers, and why least privilege matters.
...
- Core Quest (18 min): Create a simple access model: visitor, builder, mentor-admin.
...
Add a test player to a whitelist in a prepared environment. This must land within the first 7 minutes so the learner feels oriented before touching harder server concepts.
...
- Debug: learner resolves or correctly diagnoses the prepared player cannot join due whitelist or version.
...
“Operator powers should be limited because ___.”
